...
const HowItWorksAreaInServicesStyles=makeStyles(()=>({
  root:{marginBottom:isDesktopOrLaptopOrTabletScreen=>isDesktopOrLaptopOrTabletScreen ?'5%' :'10%'},
heading:{
  fontWeight:'bolder',
  font:isDesktopOrLaptopOrTabletScreen => isDesktopOrLaptopOrTabletScreen ?Headingfonts.extraSmall :Headingfonts.large
},media:{
  marginTop:isDesktopOrLaptopOrTabletScreen=> !isDesktopOrLaptopOrTabletScreen &&'5%'
}
}))
export const HowItWorksAreaInServices = () => {
  const isDesktopOrLaptopOrTabletScreen = useMediaQuery("(min-width: 960px)");
  const classes=HowItWorksAreaInServicesStyles(isDesktopOrLaptopOrTabletScreen);
  return (
    ...
    {!isDesktopOrLaptopOrTabletScreen &&   <Grid item  xs={1} ></Grid>}
      <Grid item Container md={5}  xs={10} sm={5} justifyContent='center' alignItems='center'   className={classes.media}>
        <video
